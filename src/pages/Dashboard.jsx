...
import { db, cacheManager, queueManager, backupManager } from '../lib/db'
...
export default function Dashboard() {
  ...
  const { usage, quota, persistent, refresh: refreshQuota, requestPersist } = useStorageQuota()
  ...
  async function handleClearOld() {
    ...
    if (deletedPages + deletedFiles === 0) await cacheManager.clearOldCache(7)
    refreshQuota()
    cacheManager.getCompressionStats().then(setCompressionStats)
  }
  ...
  async function handleExport() {
    try {
      ...
      setBackupMsg('Error al exportar: ' + e.message)
    } finally {
  ...
  async function handleImport(e) {
    ...
    try {
      ...
      const result = await backupManager.importAll(file)
      refreshQuota()
      setBackupMsg(`Importado: ${result.pages} páginas, ${result.files} archivos, ${result.messages} mensajes`)
    } catch (err) {
      setBackupMsg('Error al importar: ' + err.message)
    } finally {
      e.target.value = ''
    ...
    <div className="dashboard">
      ...
      <div className="storage-card">
        <div className="storage-top">
          <div>
            <div className="storage-mb">{formatSize(usage)}</div>
            <div className="storage-label">
              de {formatSize(quota)} disponibles
              {persistent === true && <span className="persist-badge"> · Protegido</span>}
            </div>
          </div>
        ...
        {persistent === false && (
          <div className="persist-warning">
            El navegador puede borrar tus datos bajo presión de memoria.{' '}
            <button className="btn-persist" onClick={requestPersist}>Proteger datos</button>
          </div>
        )}

        <div className="backup-row">
          <button className="btn-backup" onClick={handleExport}>Exportar backup</button>
          <button className="btn-backup" onClick={() => importRef.current.click()}>Importar backup</button>
      ...
      {storageWarning && (
        <div className="storage-alert warn">
          <div className="storage-alert-title">Almacenamiento casi lleno ({Math.round(storagePercent)}%)</div>
      ...
      {compressionStats && compressionStats.ratio > 0 && (
        <div className="compression-card">
          <div className="compression-title">Compresión activa</div>
      ...
      <div className="signal-card">
        ...
        {signalStrength <= 1 && isOnline && (
          <p className="signal-warning">
            Señal débil — considera pre-descargar contenido ahora
          </p>
        )}
      </div>
